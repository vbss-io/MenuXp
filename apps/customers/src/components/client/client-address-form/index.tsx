...
export const ClientAddressForm = ({
  ...
  const validateField = (field: keyof Address, value: string): string | undefined => {
    ...
    if (requiredFields.includes(field) && !value.trim()) {
      return t('Este campo é obrigatório')
    }
    if (field === 'zipCode' && value && !/^\d{5}-?\d{3}$/.test(value)) {
      return t('CEP inválido. Use o formato: 00000-000')
    }
    if (field === 'state' && value && value.length !== 2) {
      return t('Use a sigla do estado (ex: SP, RJ)')
    }
    return undefined
  }
  ...
  return (
    <S.FormContainer className={`client-address-form layout-${layout} ${className || ''}`}>
      <S.Form className="form-container" onSubmit={handleSubmit}>
        <FormInput
          id="street"
          label={t('Rua *')}
          type="text"
          placeholder={t('Digite a rua')}
        ...
        {showSaveOption && onSaveForFuture && (
          <S.SaveAddressOption className="save-address-option">
            <FormCheckbox
              id="save-address-for-future"
              label={t('Salvar este endereço para pedidos futuros')}

...
const WHATSAPP_TEMPLATE_TO_RESTAURANT_TEMPLATE_MAP: Record<
  WhatsAppTemplateKey,
  string
> = {
...
export class SendWhatsAppNotificationUsecase {
  ...
  async execute(
    ...
    const resolvedTemplate = this.resolveMessageTemplate(
      input.templateKey,
      restaurant
    );
    if (!resolvedTemplate) {
      this.logger.warn(
        "Message template not configured for WhatsApp notification",
        {
          correlationId,
      ...
      throw new Error(
        `Message template ${input.templateKey} not configured for restaurant`
      );
    }
    ...
    if (!sendResult.success) {
      ...
      throw new Error(
        `Failed to send WhatsApp notification: ${sendResult.error?.message}`
      );
    }
  ...
  private resolveMessageTemplate(
    ...
    const restaurantTemplateKey =
      WHATSAPP_TEMPLATE_TO_RESTAURANT_TEMPLATE_MAP[templateKey] ?? templateKey;
    const restaurantTemplate =
      restaurant.settings?.templates?.[restaurantTemplateKey];
  ...
  private extractTemplateVariables(template: string): string[] {
    return Array.from(template.matchAll(/#\{(.*?)\}/g)).map(
      ([, variable]) => variable
    );
  }
    ...
    switch (variable) {
      ...
      case "scheduled_for":
        return input.scheduledFor
          ? this.formatScheduledDate(input.scheduledFor)
          : "-";
      case "cancel_reason":

...
import mockMenuItemsData from '../data/mockMenuItems.json';
import mockSettingsData from '../data/mockSettings.json';
...
export function AppProvider({ children }: AppProviderProps) {
  ...
  const cancelOrder = async (orderId: number, reason: string) => {
    ...
    setOrders(prev =>
      prev.map(order =>
        order.id === orderId ? { ...order, status: 'Cancelado', cancel_reason: reason } : order
      )
    );
    ...
    setMessages(prev =>
      prev.map(message =>
        message.id === messageId 
          ? { ...message, status: 'sent', timestamp: new Date().toISOString() } 
          : message
      )
    );

...
export default function OrderCard({ order, provided, snapshot, onStatusChange, canAdvance, nextStatus }: OrderCardProps) {
  ...
  const slaColor = slaMin > 15 ? 'bg-error-100 text-error-800 border-error-500' : 
                 slaMin > 8 ? 'bg-warning-100 text-warning-800 border-warning-500' : 
    ...
    return (
      <div className="p-4 mb-3 bg-bg-dark rounded-sm shadow-sm border-l-4 border-error-500 border border-black">
        <div className="flex justify-between items-center mb-2">
          ...
          <span className="text-xs px-2 py-1 rounded-sm status-canceled">
            CANCELADO
          </span>
        </div>
  ...
  return (
    <div
      ...
      className={`card-basic mb-3 border-l-4 
        ${order.order_type === 'delivery' ? 'border-info-500' : 
          order.order_type === 'mesa' ? 'border-accent-2-500' : 
          'border-warning-500'} 

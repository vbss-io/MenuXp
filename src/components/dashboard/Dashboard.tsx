...
import { Clock, Package, AlertCircle, TrendingUp, Truck, CheckCircle, Play, Pause, Square, Target, Gift, Star } from 'lucide-react';
      ...
      case 'running':
        return (
          <div className="flex space-x-3">
            <button
              onClick={() => handleOperationAction('pause')}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div>
                  ...
                  <div className="h-60 flex items-end space-x-4 border-b border-l border-black">
                    {settings.operation_types.map((type) => {
                      const displayName = type === 'delivery' ? 'Delivery' : 
                                         type === 'mesa' ? 'Mesa' : 'Balcão';
                      const count = orders.filter(o => 
                        o.order_type.toLowerCase() === type.toLowerCase()
                      ).length;
                        ...
                        <div key={type} className="flex flex-col items-center flex-1">
                          ...
                          <div 
                            ...
                            className={`w-full border border-black ${
                              type === 'delivery' ? 'bg-blue-400' :
                              type === 'mesa' ? 'bg-green-400' : 'bg-yellow-400'
                            } rounded-t-sm`}
                          ></div>
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    {['Recebido', 'Confirmado', 'Em Produção', 'Pronto', 'Saiu para Entrega', 'Entregue', 'Cancelado'].map((status) => {
                      ...
                      return (
                        <div key={status} className="space-y-1">
                          <div className="flex justify-between text-subtitle">
                            <span>{status}</span>
                            <span>{count} pedidos ({percentage}%)</span>
                          </div>
                          <div className="w-full bg-gray-200 h-2 rounded-sm overflow-hidden border border-black">
                            <div 
                              className={`h-full ${
                                status === 'Recebido' ? 'bg-yellow-500' :
                                status === 'Confirmado' ? 'bg-blue-500' :
                                status === 'Em Produção' ? 'bg-indigo-500' :
                                status === 'Pronto' ? 'bg-green-500' :
                                status === 'Saiu para Entrega' ? 'bg-orange-500' :
                                status === 'Entregue' ? 'bg-gray-500' : 'bg-red-500'
                              }`}
